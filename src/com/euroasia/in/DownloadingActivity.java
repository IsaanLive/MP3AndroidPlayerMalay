...
public class DownloadingActivity extends ListActivity {
    ...
    public void onCreate(Bundle icicle) {
        ...
        if(totaldownloading >= 1){
        	text.setText("Jumlah muat turun: "+totaldownloading);
        }else{
        	text.setText("Jumlah muat turun: 0");
        }
        ...
        this.clearall.setOnClickListener(new OnClickListener() {
        	public void onClick(View v) {
        		...
        		toastmake("Tetapan semula dan muat turun yang gagal");
        	}
        });
		...
		public View getView(int position, View convertView, ViewGroup parent){
	    	...
	    	if(items.get(position).third == 100){
	    		...
	    		if(a == 2){
		    		...
		    		if(a == 1){
		        		progress.setText("Muat turun yang belum selesai");
		    		}else if(a == 0){
		    			progress.setText("Muat turun yang telah selesai");
		    		}else if(a == -1){
		        		progress.setText("Muat turun yang gagal");
		    		}
	    		}
	...
	protected void onListItemClick(ListView l, View v, int position, final long id) {
		...
		if(items.get(position).third != 100){
			...
			if(downloaded == -1){
				...
				builder.setMessage("Pilihan muat turun")
				       .setCancelable(false)
				       .setNegativeButton("Buang", new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
				                mDbHelper.deleteDownload(ida);
				                refreshlist();
				                toastmake("MP3 muat turun dipadam");
				           }
				       })
				       .setNeutralButton("Cuba lagi", new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
				        	   mDbHelper.updateDownloaded(ida, "1"); 
				        	   refreshlist();
				        	   toastmake("Tetapan untuk cuba lagi");
				           }
				       })
				...
				final CharSequence[] items = {"Buka MP3", "Tetapkan sebagai nada dering", "Muat turun semula", "Padam", getString(R.string.btn_cancel_name)};
				    ...
				    public void onClick(DialogInterface dialog, int item) {
				    	if(items[item] == "Buka MP3"){
				    	    try {
				    	    	...
				    	    	toastmake("Ralat memainkan MP3");
				    	        e.printStackTrace();
				    	    }
				    	}else if(items[item] == "Tetapkan sebagai nada dering"){
							...
							builder.setMessage("Anda pasti untuk padam: " + title + " ?")
							       ...
							       .setPositiveButton("Ya", new DialogInterface.OnClickListener() {
							           public void onClick(DialogInterface dialog, int id) {
							        	    ...
							        	    mDbHelper.deleteDownload(ida);
							        	    refreshlist();
							                toastmake("MP3 dipadam");
							           }
							       })
							       .setNegativeButton("Tidak", new DialogInterface.OnClickListener() {
							           public void onClick(DialogInterface dialog, int id) {
							        	   toastmake("MP3 tidak dipadam");
							               dialog.cancel();
							           }
							       });
							AlertDialog alert = builder.create();
							alert.show();
				    	}else if(items[item] == "Muat turun semula"){
				    		mDbHelper.updateDownloaded(ida, "1"); 
				    		refreshlist();
				    		toastmake("Tetapan untuk cubalagi");
				    	}
				    }
				});
				AlertDialog alert = builder.create();
				...
				builder.setMessage("Pilihan muat turun")
				       ...
				       .setNegativeButton("Buang", new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
				                mDbHelper.deleteDownload(ida);
				                refreshlist();
				                toastmake("Muat turun MP3 dipadam");
				           }
				       })
				...
				builder.setMessage("Pilihan muat turun")
				       .setCancelable(false)
				       .setNegativeButton("Berhenti memuat turun", new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
				        	   if(sid == 1){
				                ...
				                toastmake("Muatturun MP3 dihentikan");
				           }
				       })
				       .setPositiveButton(getString(R.string.btn_cancel_name), new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
    ...
    public void refreshlist(){
    	...
    	if(a.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102109, "Sedang memuat turun", 100));
    		a.moveToFirst();
    	...
    	if(a1.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102110, "Muat turun belum selesai", 100));
    	...
    	if(a11.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102111, "Muat turun selesai", 100));
    	...
    	if(a111.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102112, "Muat turun gagal", 100));
